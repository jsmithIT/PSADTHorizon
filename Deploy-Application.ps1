...
Try {
	...
	[string]$appScriptDate = '2022.08.23'
	...
	$CloseApps = "msedge=Microsoft Edge,chrome=Google Chrome,firefox=Firefox"
	...
	$64bitAppInstallName = "Horizon"
	# Application install Path.
	$64bitAppInstallPath = "${Env:ProgramFiles}\Tuolumne County\Horizon"
	# Application  install parameters.
	...
	[string[]]$appShortcutDirs = @("${Env:ProgramFiles}\Tuolumne County\Horizon", "${Env:ProgramData}\Microsoft\Windows\Start Menu\Programs", "${Env:PUBLIC}\Desktop")
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Foreach ($removeShortcutDir in $removeShortcutDirs) {
			# Check if previous versions of application install directory exist.
			If (Test-Path -Path "$removeShortcutDir\$64bitAppUninstallName*") {
				Write-Log -Message "Previous versions of $64bitAppUninstallName exist in $removeShortcutDir. Removing..."  
				# Remove previous versions of application install directory.
				Try {
					Remove-Item -Path "$removeShortcutDir\$64bitAppUninstallName*" -Force -Recurse
					Remove-Item -Path "${Env:PUBLIC}\Desktop\$64bitAppUninstallName*" -Force
				}
				# If removal failed, log results. Exit script.
				Catch [System.Exception] {
					Write-Log -Message "Removing previous versions of $64bitAppUninstallName in $removeShortcutDir failed with error: $_."
					Write-Log -Message "Exiting script with error."
					Exit-Script -ExitCode 1627
				}
				Write-Log -Message "Removing previous versions of $64bitAppUninstallName in $removeShortcutDir complete."
			}
			# Else, log results from check. 
			Else {  
				Write-Log -Message "Previous versions of $64bitAppUninstallName in $removeShortcutDir do not exist."        
			}
		}
		...
		Try {
			...
			Write-log -Message " 32bitAppInstallName: $64bitAppInstallName | 32bitAppInstallPath: $64bitAppInstallPath"
			Copy-Item -Path "$PSScriptRoot/Files" -Destination "$64bitAppInstallPath" -Force -Recurse
		...
		Foreach ($appShortcutDir in $appShortcutDirs) {
			Try {
				...
				New-Shortcut -Path "$appShortcutDir\$64bitAppInstallName.lnk" -TargetPath "CONFIDENTIAL" -IconLocation "$64bitAppInstallPath\Assets\Horizon_icon.ico" -WindowStyle "Maximized" -Description "Horizon"
			}
